...
test("papers analyze score and refine CLI write analysis artifacts", async () => {
  ...
  try {
    ...
    const revisedIdea = await readFile(join(output, "docs/proposal/revised_idea.md"), "utf8");
    assert.match(revisedIdea, /Revised Idea/);
    assert.match(revisedIdea, /One-Sentence Claim/);
    assert.match(revisedIdea, /Central Hypothesis/);
    assert.match(revisedIdea, /Why This Is Different From Prior Work/);
    assert.match(revisedIdea, /What Must Be Proven/);
    assert.match(revisedIdea, /Expected Contribution Type/);
    assert.doesNotMatch(revisedIdea, /^## Baselines/m);
    const strictPlan = await readFile(join(output, "docs/proposal/strict_execution_plan.md"), "utf8");
    assert.match(strictPlan, /^# 12-Week Execution Plan/);
    assert.match(strictPlan, /Week \| Goal \| Tasks \| Deliverables \| Acceptance Criteria \| Risks/);
    assert.match(strictPlan, /docs\/relative_work\/search_plan\.md/);
    assert.doesNotMatch(strictPlan, /Reproducibility Commands \/ Paths/);
    assert.doesNotMatch(strictPlan, /^## One-Sentence Claim/m);
    assert.doesNotMatch(strictPlan, /^\s*\{/m);
    const solutionDesign = await readFile(join(output, "docs/proposal/solution_design.md"), "utf8");
    assert.match(solutionDesign, /^# Feasible Solution Design\r?\n\r?\n## System \/ Method Overview/);
    assert.match(solutionDesign, /System \/ Method Overview/);
    assert.match(solutionDesign, /Reproducibility Plan/);
    assert.doesNotMatch(solutionDesign, /^## One-Sentence Claim/m);
    assert.doesNotMatch(solutionDesign, /^\s*\{/m);
