...
test("papers analyze score and refine CLI write analysis artifacts", async () => {
  ...
  try {
    ...
    assert.match(strictPlan, /Strict Execution Plan/);
    assert.match(strictPlan, /12-Week Execution Table/);
    assert.match(strictPlan, /Reproducibility Commands \/ Paths/);
    assert.doesNotMatch(strictPlan, /^\s*\{/m);
    const solutionDesign = await readFile(join(output, "docs/proposal/solution_design.md"), "utf8");
    assert.match(solutionDesign, /Solution Design/);
    assert.match(solutionDesign, /Proof Obligations/);
